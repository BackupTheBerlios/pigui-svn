...
public:
	...
	int connect( const M& p_method ) {
		...
		while (c) {
			...
			if (c->method==m) {
				//GUI_PRINT_ERROR("Already connected");
	...
	void disconnect(int p_count) {
		...
		if (!c) {
			GUI_PRINT_ERROR("Invalid connection count.");
			return;
		};
...
//		template<class P1,class P2,class P3,class P4>
	...
	void clear() {
		...
		if (call_ref>0) {
			...
			GUI_PRINT_ERROR(" DO NOT CLEAR A SIGNAL IN THE MIDDLE OF A CALL() ");
	...
	~Signal() {
		...
		if (call_ref>0) {
			...
			GUI_PRINT_ERROR(" DO NOT DELETE A SIGNAL IN THE MIDDLE OF A CALL() ");
